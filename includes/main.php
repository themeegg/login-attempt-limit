...
function limit_login_setup() {
	load_plugin_textdomain( 'login-attempt-limit', false
...
function limit_login_get_address( $type_name = '' ) {
	 ...
	 */
	if ( empty( $type_name ) && $type == LAL_PROXY_ADDR
	     && isset( $_SERVER[ LAL_DIRECT_ADDR ] )
	) {
		...
		return $_SERVER[ LAL_DIRECT_ADDR ];
	}
...
function limit_login_notify_email( $user ) {
	...
	if ( ! isset( $retries[ $ip ] ) ) {
		...
		$when     = sprintf( _n( '%d hour', '%d hours', $time, 'login-attempt-limit' ), $time );
	} else {
		/* normal lockout */
		$count    = $retries[ $ip ];
		$lockouts = floor( $count / limit_login_option( 'allowed_retries' ) );
		$time     = round( limit_login_option( 'lockout_duration' ) / 60 );
		$when     = sprintf( _n( '%d minute', '%d minutes', $time, 'login-attempt-limit' ), $time );
	}
	...
	$message = sprintf( __( "%d failed login attempts (%d lockout(s)) from IP: %s"
		...
		, $count, $lockouts, $ip );
	if ( $user != '' ) {
		$message .= sprintf( __( "Last user attempted: %s", 'login-attempt-limit' )
		                     . "\r\n\r\n", $user );
	}
	if ( $whitelisted ) {
		$message .= __( "IP was NOT blocked because of external whitelist.", 'login-attempt-limit' );
	} else {
		$message .= sprintf( __( "IP was blocked for %s", 'login-attempt-limit' ), $when );
	}
...
function limit_login_error_msg() {
	...
	$msg = __( '<strong>ERROR</strong>: Too many failed login attempts.', 'login-attempt-limit' ) . ' ';

	if ( ! is_array( $lockouts ) || ! isset( $lockouts[ $ip ] ) || time() >= $lockouts[ $ip ] ) {
		/* Huh? No timeout active? */
		$msg .= __( 'Please try again later.', 'login-attempt-limit' );
	...
	if ( $when > 60 ) {
		$when = ceil( $when / 60 );
		$msg .= sprintf( _n( 'Please try again in %d hour.', 'Please try again in %d hours.', $when, 'login-attempt-limit' ), $when );
	} else {
		$msg .= sprintf( _n( 'Please try again in %d minute.', 'Please try again in %d minutes.', $when, 'login-attempt-limit' ), $when );
	}
...
function limit_login_retries_remaining_msg() {
	...
	return sprintf( _n( "<strong>%d</strong> attempt remaining.", "<strong>%d</strong> attempts remaining.", $remaining, 'login-attempt-limit' ), $remaining );
}
...
function limit_login_fixup_error_messages( $content ) {
	...
	if ( $limit_login_nonempty_credentials && $count > $my_warn_count ) {
		...
		$content = __( '<strong>ERROR</strong>: Incorrect username or password.', 'login-attempt-limit' ) . "<br />\n";
...
function limit_login_guess_proxy() {
	return isset( $_SERVER[ LAL_PROXY_ADDR ] )
		? LAL_PROXY_ADDR : LAL_DIRECT_ADDR;
}
...
function limit_login_sanitize_variables() {
	...
	$args         = explode( ',', limit_login_option( 'lockout_notify' ) );
	$args_allowed = explode( ',', LAL_LOCKOUT_NOTIFY_ALLOWED );
	$new_args     = array();
	foreach ( $args as $a ) {
	...
	if ( limit_login_option( 'client_type' ) != LAL_DIRECT_ADDR
	     && limit_login_option( 'client_type' ) != LAL_PROXY_ADDR
	) {
		$limit_login_options['client_type'] = LAL_DIRECT_ADDR;
	}
}
...
function limit_login_admin_menu() {
	...
	// Modern WP?
	if ( version_compare( $wp_version, '3.0', '>=' ) ) {
		add_options_page( 'Limit Login Attempts', 'Limit Login Attempts', 'manage_options', 'login-attempt-limit', 'limit_login_option_page' );
	...
	if ( function_exists( "get_current_site" ) ) {
		add_submenu_page( 'wpmu-admin.php', 'Limit Login Attempts', 'Limit Login Attempts', 9, 'login-attempt-limit', 'limit_login_option_page' );

		return;
	}

	// Older WP
	add_options_page( 'Limit Login Attempts', 'Limit Login Attempts', 9, 'login-attempt-limit', 'limit_login_option_page' );
}
...
function limit_login_show_log( $log ) {
	...
	echo( '<tr><th scope="col">' . _x( "IP", "Internet address", 'login-attempt-limit' ) . '</th><th scope="col">' . __( 'Tried to log in as', 'login-attempt-limit' ) . '</th></tr>' );
	foreach ( $log as $ip => $arr ) {
		echo( '<tr><td class="limit-login-ip">' . $ip . '</td><td class="limit-login-max">' );
		$first = true;
		foreach ( $arr as $user => $count ) {
			$count_desc = sprintf( _n( '%d lockout', '%d lockouts', $count, 'login-attempt-limit' ), $count );
			if ( ! $first ) {
...
function limit_login_option_page() {
	...
	if ( count( $_POST ) > 0 ) {
		check_admin_referer( 'login-attempt-limit-options' );
	}
	...
	if ( isset( $_POST['clear_log'] ) ) {
		...
		echo '<div id="message" class="updated fade"><p>'
		     . __( 'Cleared IP log', 'login-attempt-limit' )
		     . '</p></div>';
	}
	...
	if ( isset( $_POST['reset_total'] ) ) {
		...
		echo '<div id="message" class="updated fade"><p>'
		     . __( 'Reset lockout count', 'login-attempt-limit' )
		     . '</p></div>';
	}
	...
	if ( isset( $_POST['reset_current'] ) ) {
		...
		echo '<div id="message" class="updated fade"><p>'
		     . __( 'Cleared current lockouts', 'login-attempt-limit' )
		     . '</p></div>';
	}
	...
	if ( isset( $_POST['update_options'] ) ) {
		...
		echo '<div id="message" class="updated fade"><p>'
		     . __( 'Options changed', 'login-attempt-limit' )
		     . '</p></div>';
	}
	...
	$client_type_direct = $client_type == LAL_DIRECT_ADDR ? ' checked ' : '';
	$client_type_proxy  = $client_type == LAL_PROXY_ADDR ? ' checked ' : '';

	$client_type_guess = limit_login_guess_proxy();

	if ( $client_type_guess == LAL_DIRECT_ADDR ) {
		$client_type_message = sprintf( __( 'It appears the site is reached directly (from your IP: %s)', 'login-attempt-limit' ), limit_login_get_address( LAL_DIRECT_ADDR ) );
	} else {
		$client_type_message = sprintf( __( 'It appears the site is reached through a proxy server (proxy IP: %s, your IP: %s)', 'login-attempt-limit' ), limit_login_get_address( LAL_DIRECT_ADDR ), limit_login_get_address( LAL_PROXY_ADDR ) );
	}
	$client_type_message .= '<br />';

	$client_type_warning = '';
	if ( $client_type != $client_type_guess ) {
		$faq = 'http://wordpress.org/extend/plugins/login-attempt-limit/faq/';

		$client_type_warning = '<br /><br />' . sprintf( __( '<strong>Current setting appears to be invalid</strong>. Please make sure it is correct. Further information can be found <a href="%s" title="FAQ">here</a>', 'login-attempt-limit' ), $faq );
	}
	...
	?>
    <div class="wrap">
        <h2><?php echo __( 'Limit Login Attempts Settings', 'login-attempt-limit' ); ?></h2>
        <h3><?php echo __( 'Statistics', 'login-attempt-limit' ); ?></h3>
        <form action="options-general.php?page=login-attempt-limit" method="post">
			<?php wp_nonce_field( 'login-attempt-limit-options' ); ?>
            <table class="form-table">
                <tr>
                    <th scope="row" valign="top"><?php echo __( 'Total lockouts', 'login-attempt-limit' ); ?></th>
                    <td>
						<?php if ( $lockouts_total > 0 ) { ?>
                            <input class="button button-primary button-large" name="reset_total"
                                   value="<?php echo __( 'Reset Counter', 'login-attempt-limit' ); ?>" type="submit"/>
							<?php echo sprintf( _n( '%d lockout since last reset', '%d lockouts since last reset', $lockouts_total, 'login-attempt-limit' ), $lockouts_total ); ?>
						<?php } else {
							echo __( 'No lockouts yet', 'login-attempt-limit' );
						} ?>
                    </td>
                </tr>
				<?php if ( $lockouts_now > 0 ) { ?>
                    <tr>
                        <th scope="row" valign="top"><?php echo __( 'Active lockouts', 'login-attempt-limit' ); ?></th>
                        <td>
                            <input name="reset_current"
                                   value="<?php echo __( 'Restore Lockouts', 'login-attempt-limit' ); ?>"
                                   type="submit"/>
							<?php echo sprintf( __( '%d IP is currently blocked from trying to log in', 'login-attempt-limit' ), $lockouts_now ); ?>
                        </td>
                    </tr>
				<?php } ?>
            </table>
        </form>
        <h3><?php echo __( 'Options', 'login-attempt-limit' ); ?></h3>
        <form action="options-general.php?page=login-attempt-limit" method="post">
			<?php wp_nonce_field( 'login-attempt-limit-options' ); ?>
            <table class="form-table">
                <tr>
                    <th scope="row" valign="top"><?php echo __( 'Lockout', 'login-attempt-limit' ); ?></th>
                    <td>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'allowed_retries' ) ); ?>"
                               name="allowed_retries"/> <?php echo __( 'allowed retries', 'login-attempt-limit' ); ?>
                        <br/>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'lockout_duration' ) / 60 ); ?>"
                               name="lockout_duration"/> <?php echo __( 'minutes lockout', 'login-attempt-limit' ); ?>
                        <br/>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'allowed_lockouts' ) ); ?>"
                               name="allowed_lockouts"/> <?php echo __( 'lockouts increase lockout time to', 'login-attempt-limit' ); ?>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'long_duration' ) / 3600 ); ?>"
                               name="long_duration"/> <?php echo __( 'hours', 'login-attempt-limit' ); ?> <br/>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'valid_duration' ) / 3600 ); ?>"
                               name="valid_duration"/> <?php echo __( 'hours until retries are reset', 'login-attempt-limit' ); ?>
                    </td>
                </tr>
                <tr>
                    <th scope="row" valign="top"><?php echo __( 'Site connection', 'login-attempt-limit' ); ?></th>
                    <td>
						<?php echo $client_type_message; ?>
                        <label>
                            <input type="radio" name="client_type"
								<?php echo $client_type_direct; ?> value="<?php echo LAL_DIRECT_ADDR; ?>"/>
							<?php echo __( 'Direct connection', 'login-attempt-limit' ); ?>
                        </label>
                        <label>
                            <input type="radio" name="client_type"
								<?php echo $client_type_proxy; ?> value="<?php echo LAL_PROXY_ADDR; ?>"/>
							<?php echo __( 'From behind a reversy proxy', 'login-attempt-limit' ); ?>
                        </label>
						<?php echo $client_type_warning; ?>
                    </td>
                </tr>
                <tr>
                    <th scope="row" valign="top"><?php echo __( 'Handle cookie login', 'login-attempt-limit' ); ?></th>
                    <td>
                        <label><input type="radio" name="cookies" <?php echo $cookies_yes; ?>
                                      value="1"/> <?php echo __( 'Yes', 'login-attempt-limit' ); ?></label>
                        <label><input type="radio" name="cookies" <?php echo $cookies_no; ?>
                                      value="0"/> <?php echo __( 'No', 'login-attempt-limit' ); ?></label>
                    </td>
                </tr>
                <tr>
                    <th scope="row" valign="top"><?php echo __( 'Notify on lockout', 'login-attempt-limit' ); ?></th>
                    <td>
                        <input type="checkbox" name="lockout_notify_log" <?php echo $log_checked; ?>
                               value="log"/> <?php echo __( 'Log IP', 'login-attempt-limit' ); ?><br/>
                        <input type="checkbox" name="lockout_notify_email" <?php echo $email_checked; ?>
                               value="email"/> <?php echo __( 'Email to admin after', 'login-attempt-limit' ); ?>
                        <input type="text" size="3" maxlength="4"
                               value="<?php echo( limit_login_option( 'notify_email_after' ) ); ?>"
                               name="email_after"/> <?php echo __( 'lockouts', 'login-attempt-limit' ); ?>
                    </td>
                </tr>
            </table>
            <p class="submit">
                <input name="update_options" class="button button-primary button-large"
                       value="<?php echo __( 'Change Options', 'login-attempt-limit' ); ?>"
                       type="submit"/>
            </p>
        </form>
		<?php
		...
		if ( is_array( $log ) && count( $log ) > 0 ) {
			?>
            <h3><?php echo __( 'Lockout log', 'login-attempt-limit' ); ?></h3>
            <form action="options-general.php?page=login-attempt-limit" method="post">
				<?php wp_nonce_field( 'login-attempt-limit-options' ); ?>
                <input type="hidden" value="true" name="clear_log"/>
                <p class="submit">
                    <input name="submit" value="<?php echo __( 'Clear Log', 'login-attempt-limit' ); ?>"
                           type="submit"/>
                </p>
            </form>
